...
std::shared_ptr<DRL::Texture2D> PBOBench::BenchUpload(TextureUploadingMode mode) {
  ...
  {
    static auto spot0 = std::make_shared<DRL::Texture2D>(originalTexture, 1, false, false);
    static auto spot1 = std::make_shared<DRL::Texture2D>(DarkerTexture, 1, false, false);
  ...
  {
    spdlog::info("Time for Syncchronos Uploding Texture: {}ms", latency);
  }
  return Res;
}
...
void PBOBench::render() {
  ...
  ImGui::NewFrame();
  {
    ...
    if(BenchUploading)
    {
        ...
        ImGui::Combo("UpLoading Mode", (int*)&mUploadingMode, modes, IM_ARRAYSIZE(modes));
    }
    ImGui::End();
  }
  ImGui::Render();
...
void PBOBench::update_model_matrics() {
  ...
  glNamedBufferSubData(modelMatricsVBO,0,DrawNumbers* sizeof(glm::mat4),modelMatrics.data());
}
int main() {
  // spdlog init
