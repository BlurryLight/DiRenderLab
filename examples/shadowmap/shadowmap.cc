...
void ShadowMapRender::setup_states() {
  ...
  // -------------------------
  shader = DRL::make_program(resMgr.find_path("shadow_mapping.vert"),
                             resMgr.find_path("shadow_mapping.frag"));
  simpleDepthShader =
      DRL::make_program(resMgr.find_path("shadow_mapping_depth.vert"),
                        resMgr.find_path("shadow_mapping_depth.frag"));

  DepthMapShader = DRL::make_program(resMgr.find_path("debug_quad.vert"),
                                     resMgr.find_path("debug_quad_depth.frag"));

  LightShader = DRL::make_program(resMgr.find_path("shadow_mapping.vert"),
...
void ShadowMapRender::render() {
  ImGui::NewFrame();
  {
    ...
    if (ImGui::CollapsingHeader("PCF")) {
      ImGui::SliderFloat("PCF filter size", &uPCFFilterSize, 0.0f, 20.0f,
                         "%.3f");
    }

    if (ImGui::CollapsingHeader("PCSS")) {
      ImGui::SliderFloat("PCSS block size", &uPCSSBlockSize, 0.0f, 20.0f,
                         "%.3f");
      ImGui::SliderFloat("PCSS light size", &uPCSSLightSize, 0.0f, 100.0f,
                         "%.3f");
    }
    ImGui::End();
  }
